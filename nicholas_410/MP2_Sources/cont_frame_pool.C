...
unsigned long ContFramePool::get_frames(unsigned int _n_frames)
{
    ...
    assert(_nframes <= info_frame_no FRAME_SIZE * 8);
                                      ...
                                      unsigned long _n_frames)
{
    ...
    unsigned char flip_bit = ((4 - (_frame_no - base_frame_no) % 4) * 2) - 1;
    
    assert((bitmap[bitmap_index] & (1 << flip_bit)) != );
...
void ContFramePool::mark_inaccessible(unsigned long _frame_no)
{
    ...
    unsigned int bitmap_index = (_frame_no - base_frame_no) / 4;
    unsigned char flip_bit = ((4 - (_frame_no - base_frame_no) % 4) * 2);
    
    // Is the frame being used already?
    assert((bitmap[bitmap_index] & (1 << flip_bit)) != );
    
    // Update bitmap
    bitmap[bitmap_index] ^=  (1 << flip_bit);
    nFreeFrames--;
}
...
{
    return _n_frames / 16k + (_n_frames % 16k > 0 ? 1 : 0)
}
